...
public class BlockItemFeatures {
    ...
    public static void registerCompostables(BiConsumer<ItemLike, Float> consumer) {
        ...
        compostItems(consumer, 0.5F, NetherDescentBlocks.EMBUR_SPROUTS.get(), NetherDescentBlocks.SYTHIAN_SPROUTS.getBlock(), NetherDescentBlocks.ARISIAN_SPROUTS.getBlock(), NetherDescentBlocks.TALL_ARISIAN_SPROUTS.get());
        ...
        compostItems(consumer, 0.65F, NetherDescentBlocks.EMBUR_ROOTS.getBlock(), NetherDescentBlocks.TALL_EMBUR_ROOTS.get(), NetherDescentBlocks.TALL_CRIMSON_ROOTS.get(),
                NetherDescentBlocks.FUNGAL_BULBS.get(), NetherDescentBlocks.TALL_CRIMSON_FUNGI.get(), NetherDescentBlocks.SYTHIAN_ROOTS.getBlock(), NetherDescentBlocks.ARISIAN_DANDELIONS.get(),
        ...
        if (!level.isClientSide() && entity instanceof ServerPlayer serverPlayer) {
            if (placed.is(BlockTags.FLOWERS))
                if (!level.getEntitiesOfClass(Hornet.class, new AABB(pos).inflate(5.0)).isEmpty())
                    NetherDescentCriteriaTriggers.PLACE_FLOWER_NEAR_HORNET.trigger(serverPlayer);
        }
    }
}
